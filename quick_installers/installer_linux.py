...
class Installer():
    ...
    def __init__(self):
        ...
        if self.windowsCheck() or self.osxCheck():
            input("This is the installer for Linux.")
            sys.exit()
        ...
        for newItem in self.newItems:
            try:
                ...
                if not self.itemTypes[itemType]['type'] == 'weblink':
                    ...
                    linkPath = self.cwd+'Media/'+typeDir+'/'+itemName+'.desktop'
                    ...
                    try:
                        ...
                        if self.itemTypes[itemType]['type'] == 'weblink':
                            if termParent in ("", None, 0):
                                linkPath = self.cwd+taxonomyDir+'/'+termName+'/'+typeDir+'/'+itemName+".desktop"
                            else:
                                ...
                                linkPath = self.cwd+taxonomyDir+'/'+termParentRoot+'/'+typeDir+'/'\
                                    + termParentsJoined+termName+'/'+itemName+".desktop"
        ...
        try:
            ...
            if os.path.exists(linkPath):
                if self.overwriteLinks is True:
                    ...
                    file.write("Name="+itemName+'\n')
                    file.write("Type=Link"+'\n')
                    file.write("URL="+itemSource)
                    file.close()
