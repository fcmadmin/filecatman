...
class Installer():
    ...
    def __init__(self):
        ...
        if not self.windowsCheck():
            input("This is the installer for Windows.")
            sys.exit()
        ...
        for newItem in self.newItems:
            try:
                ...
                if not self.itemTypes[itemType]['type'] == 'weblink':
                    ...
                    if not self.createDesktopFile(linkPath, itemSource):
                        return False
                try:
                    ...
                    if not 0 in (year, month, day):
                        ...
                        if self.itemTypes[itemType]['type'] == 'weblink':
                            ...
                            if not self.createDesktopFile(timeLink, itemSource):
                                return False
                        else:
                            timeLink = self.cwd+'Time\\'+timeYear+'\\'+timeMonth+'\\'\
                                + typeDir+'\\'+itemName+'.lnk'
                ...
                for relation in newItem['relations']:
                    try:
                        ...
                        if self.itemTypes[itemType]['type'] == 'weblink':
                            ...
                            if termParent in ("", None, 0):
                                levels = 3
                                linkPath = self.cwd+taxonomyDir+'\\'+termName+'\\'+typeDir+'\\'+itemName+'.lnk'
                            else:
                                termParentRoot, termParentsJoined, parentLevels = self.returnTermParents(
                                    termParent, termTaxonomy)
                                levels = 3+parentLevels
                                linkPath = self.cwd+taxonomyDir+'\\'+termParentRoot+'\\'+typeDir+'\\'\
                                    + termParentsJoined+termName+'\\'+itemName+'.lnk'
    ...
    def createDesktopFile(self, linkPath, itemSource):
        try:
            ...
            if os.path.exists(linkPath):
                if self.overwriteLinks is True:
                    newShortcut = self.wshShell.CreateShortcut(linkPath)
            ...
            self.logErrors.error("File '{}' probably contains an illegal character. {}".format(linkPath, str(e)))
            self.errorCount += 1
            return False
        except BaseException as e:
